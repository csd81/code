...
// void jarmuKiirSima(Jarmu j) { j.kiir(); }
void jarmuKiirVirtualis(const Jarmu &j) { j.kiir(); }
...
int main()
{
    ...
    Tarolo t1;
    t1.megjelenit();
    t1.beallit(2,new Hajo(3,7,4));
    t1.beallit(4,new Hajo(8,1,4));
    t1.megjelenit();
    t1.beallit(6,new Repulo(1,2,5));
    t1.beallit(2,new Repulo(1,9,4));
    t1.megjelenit();
