...
#include <stdio.h>
#include <stdlib.h>
#include <string.h> #define DEFAULT_INPUTFILE "date.txt" #define DOT '.' 
#define MAX_LENGTH 10 struct TDate { char Year[MAX_LENGTH];
char Month[MAX_LENGTH];
char Day[MAX_LENGTH];
};
void InitDate(struct TDate * Date) { Date->Year[0] = Date->Month[0] = 
Date->Day[0] = 0;
...
} void CheckDate(struct TDate * Date) { int errnum = 
CheckNumAndDots(Date->Year, "Year");
