...
for (idxI=0;
idxI<size;
idxI++) { printf("row[%d]=", idxI);
scanf("%lf", &row[idxI]);
} for (idxI=0;
idxI<size;
idxI++) { printf("column[%d]=", idxI);
scanf("%lf", &column[idxI]);
} printf("\nHere comes the multiplying table:\n");
printf(" |");
for (idxI=0;
idxI<size;
idxI++) { printf("%12.4lg|", row[idxI]);
} printf("\n");
for (idxI=0;
idxI<(5+1)*(12+1);
idxI++) { first column, 12 width, +1 '|' character printf("-");
} } for (idxI=0;
idxI<size;
idxI++) { printf("\n%12.4lg|", column[idxI]);
for (idxJ=0;
idxJ<size;
idxJ++) { printf("%12.4lg|", row[idxI]*column[idxJ]);
} } return 0;

// Let us implement this code in c++

// 2.10.1. Create a custom multiplication table of size 5*5, which is displayed on the character screen! Separate the values of the first row and column, which show what to multiply with what, from the products with lines! The values of the first row and first column are requested from the keyboard!
...
2.10.2. A szorzótáblához hasonló módon valósítsuk meg az egyedi összeadó táblát! Az első sor és az első oszlop értékeit szöveg fájlból olvassuk be! 2.11. Másodfokú egyenlet
2.10.2.
#include <stdio.h> #define SIZE 3 #define COLUMN_WIDTH 12 int main() { short int sum;
...
idxJ < SIZE;
idxJ++) { printf("column[%d]=", idxJ);
scanf("%d", &column[idxJ]);
} printf("\nHere comes the adder table:\n");
printf("%*s|", COLUMN_WIDTH, " ");
for (idxI = 0;
idxI < SIZE;
idxI++) printf("%*d|", COLUMN_WIDTH,row[idxI]);
printf("\n");
for (idxI = 0;
idxI < (SIZE+1)*(COLUMN_WIDTH+1);
idxI++) printf("-");
for (idxJ = 0;
idxJ < SIZE;
...
for (idxI = 0;
idxI < SIZE;
idxI++) { sum = row[idxI] + column[idxJ];
if (row[idxI] > 0 && column[idxJ] > 0 && sum < 0) printf("%*s", COLUMN_WIDTH+1, "x");
else if (row[idxI] < 0 && column[idxJ] < 0 && sum > 0) printf("%*s", COLUMN_WIDTH+1, "x");
else printf("%*d", COLUMN_WIDTH+1, sum);
} } printf("\n");
return 0;
} 
...
2.28.1. Írjon programot, amely a konzolos képernyőn szemléltet egy int*** mutatót és az általa mutatott értékeket! Írja ki a jelenlévő változók címét, értékét és hogy milyen alternatív módon lehet hivatkozni a változókra! 2.2. ábra: Lehetséges képernyőkép
2.28.2
